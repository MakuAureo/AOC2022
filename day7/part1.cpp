...
//Part1.cpp special function
unsigned int Directory::sumSizesLessThan(unsigned int size) {
  static unsigned int sizeSum;

  if (memorySize < size) { sizeSum += memorySize; }
  for (int i = 0; i < directoryCount; i++) { directories[i]->sumSizesLessThan(size); }
...
int main() {
  ...
  //Output result
  std::cout << root.sumSizesLessThan(100000) << std::endl;
